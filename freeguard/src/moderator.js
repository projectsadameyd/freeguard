...
//  7 layers: normalize → regex → normalized match → spam → links
//  → invites → caps → AI analysis
//  EchoBastion Group
// ============================================================
...
- "n1gg3r", "n i g g a", "ниггер" (Cyrillic), "𝒏𝒊𝒈𝒈𝒆𝒓" (Mathematical) → HATE_SPEECH
...
//  Strips all obfuscation to reveal the underlying word
// ══════════════════════════════════════════════════════════════

// Unicode homoglyph map (Cyrillic, Greek, mathematical, etc.)
const HOMOGLYPH_MAP = {
  '\u0430': 'a', '\u0431': '6', '\u0432': 'b', '\u0433': 'r', '\u0434': 'd',
  '\u0435': 'e', '\u0436': 'x', '\u0437': '3', '\u0438': 'u', '\u0439': 'u',
  '\u043A': 'k', '\u043B': 'n', '\u043C': 'm', '\u043D': 'h', '\u043E': 'o',
...
// Full-width Latin → ASCII (UFF01-FF5E → 0021-007E)
...
function normalizeText(text) {
  ...
  for (const char of text) {
    // Full-width → ASCII
    let mapped = fullWidthToAscii(char);
    if (mapped !== char) { result += mapped.toLowerCase(); continue; }
  ...
  // Apply leet speak mapping BEFORE stripping
  result = result.replace(/[0-9@$!+*?]/g, c => LEET_MAP[c] || c);

  // Remove all non-alphanumeric
  result = result.replace(/[^a-z0-9]/g, '');

  // Collapse repeated chars (fuckk → fuck, shiiiit → shit)
...
// ══════════════════════════════════════════════════════════════
//  BLOCKED WORD LIST (checked against normalized text)
//  This is the primary defense — no regex tricks can bypass
// ══════════════════════════════════════════════════════════════
...
const BLOCKED_WORDS = {
  HATE_SPEECH: [
    ...
    'tranny', 'trannies', 'nazi', 'nazis', 'trump',
  ...
  PROFANITY: [
    ...
    'fuuck', 'fick', 'fcuk', 'cuck', 'slut', 'dik',
    'shyt', 'bytch', 'pussi', 'cum', 'cumming', 'cumshot',
  ],
  VIOLENT_THREAT: [
  ...
  SEXUAL_CONTENT: [
    ...
    'dick pic', 'dickpic',
    'boobs', 'tits', 'titty', 'titties', 'breasts',
    'penis', 'vagina', 'genitals',
    'oral', 'anal', 'handjob', 'blowjob', 'titjob',
    'sex tape', 'sex video',
    'henentai', 'hentai',
    'xxx', 'nsfw', 'adult content',
    'erotic', 'erotica', 'sexy',
...
// Build a single regex from all blocked words for fast matching
...
async function moderateMessage(content, userId, guildId) {
  ...
  const blockedMatch = normalized.match(BLOCKED_REGEX);
  if (blockedMatch) {
    const word = blockedMatch[0].toLowerCase();
  ...
  // ── Layer 7: AI analysis (catches anything regex missed) ──
  try {
